...
import logic.SummaryGet;
import logic.JQSFileFilter;
 ...
 * for the logic.JQuestions application. This is an 'exam simulator' type
 * program which reads files created and edited using the logic.JQuestions
 * Editor program.
 *
...
public class JQuestionsGUI extends JPanel {
    ...
    // Configuration Settings variables for this logic.Question pool.
    ...
    // Main UI compponents.	
    private JQMultiChoiceQuestionScreen multiChoicePanel;
    private JQDragAndDropQuestionScreen dndPanel;
    ...
    private final JQConfigurationWindow configWindow;
    private JQReportSummaryWindow reportWindow;
     ...
     *
     * @param jq       - Reference to main logic.JQuestions application.
    ...
    public JQuestionsGUI(JQuestions jq, boolean isApplet, boolean isDemo) {
        ...
        this.isDemo = isDemo;
        configWindow = new JQConfigurationWindow(this);
    ...
    private void updateCountLabel() {
        ...
        if (nQuestions <= 0) {
            countLabel.setText("logic.Question " + (qIdsIndex + 1) +
                    " of " + size);
            return;
     ...
     * logic.Question which will be available to the user in 'Quiz' mode but not
     * 'Exam' mode.
     *
     ...
     */
    private boolean isCorrect(Question question, JQUserAnswer answer) {
        boolean retVal = true;

        if (question.getQuestionType() == QuestionType.MULTI_CHOICE) {

            if (multiChoicePanel.isCorrect(question) == false) {
                retVal = false;
            }
        } else if (question.getQuestionType() == QuestionType.DRAG_N_DROP) {

            if (dndPanel.isCorrect(question) == false) {
                retVal = false;
            }
        }
    ...
    private void hintButtonActionPerformed() {
        ...
        if ((hintText == null) || (hintText.length() <= 0)) {
            ...
            String response =
                    MultiLineTextInputDialog.showInputDialog("Hint text", displayHintText, false);
        } else {
            String response =
                    MultiLineTextInputDialog.showInputDialog("Hint text", hintText, false);
        }
    ...
    private void explainButtonActionPerformed() {
        ...
        if (currentlyDisplayedQuestion.getQuestionType() == QuestionType.MULTI_CHOICE) {
            ...
            for (Integer i : correct) {
                labels.add(JQMultiChoiceQuestionScreen.generateLabel(i - 1));
            }
            ...
            if (allEmpty) {
                ...
                for (int i = 1; i <= options; i++) {
                    ...
                    displayExplainText.append('\n' + JQMultiChoiceQuestionScreen.generateLabel(i - 1) + " " + correctness + ": " + str);
                }
            }


            String response = MultiLineTextInputDialog.showInputDialog(
    ...
    private void menuAboutActionPerformed() {
        JQAboutDialog jQAboutDialog = JQAboutDialog.getInstance();
        jQAboutDialog.setVisible(true);
    }
     ...
     * File Open
     */
    private void menuFileOpenActionPerformed() {
    ...
    private void initQuestionPool(final String fileName) {
            ...
            if (question.getQuestionType() == QuestionType.DRAG_N_DROP) {
                ...
                for (int i1 = 0; i1 < doc.getLength(); i1++) {
                    ...
                    if (comp != null && (comp instanceof JTextField)) {
                        ...
                        // Add drop listener
                        new DropTarget(jtf, new JQJTextFieldDropTargetListener(jtf));
        ...
        if (isChangeAllowed) {
            ...
            // else
            // - we use the logic.Question Pool values from the .jqs file.
            //
            if (pressedOK) {
                // updateConfig vars
                updateConfSettings(qPool);
        ...
        jQUserAnswers = jQuestions.getUserAnswers();
        ...
        for (int i = 0; i < qPool.getSize(); i++) {
            Question question = qPool.getQuestion(i);
            JQUserAnswer newUserAnswer = new JQUserAnswer(question, i);
            jQUserAnswers.add(newUserAnswer);
            question.shuffleAnswers();
        }


        // Questions are to be drawn of the logic.Question Pool and asked
        // in a random order.
    ...
    private void createMenuBar() {
        ...
        // ---- menuAbout ----
        menuAbout.setText("About logic.JQuestions");
    ...
    public JPanel createSouthPanel() {
        ...
        hintButton.setToolTipText("<html>Display any hint-text for current logic.Question.</html>");
        ...
        explainButton.setToolTipText("<html>Display answer and explanation text <br>for current logic.Question.</html>");
    ...
    private void initComponents() {
        ...
        multiChoicePanel = new JQMultiChoiceQuestionScreen(northPanel, southPanel);
        dndPanel = new JQDragAndDropQuestionScreen(northPanel, southPanel);
