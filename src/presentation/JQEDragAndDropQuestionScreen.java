...
public class JQEDragAndDropQuestionScreen extends JQEEditorQuestionScreen {

	private JTextPane mainTextPane;
	private JTextArea questionTextArea;
	private final ArrayList<JTextField> optionsFields = new ArrayList<JTextField>();
	 ...
	 * @param buttonPanel
	 */
	JQEDragAndDropQuestionScreen(JPanel headerPanel, JPanel buttonPanel) {
	...
	public void createDnDPanel() {
		...
		new DropTarget(mainTextPane, new JQETextDropTargetListener(mainTextPane));
		...
		for (int i = 0; i < MAX_NUMBER_OF_OPTIONS; i++) {
			...
			jtf.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					optionsFieldMouseClicked();
				}

				@Override
				public void mousePressed(MouseEvent e) {
					optionsFieldMousePressed(jtf);
				}
			});
		...
		for (int i = 0; i < doc.getLength(); i++) {
			...
			if (component != null && (component instanceof JTextField)) {
				...
				if (questionString.compareTo("") == 0) {
					JOptionPane.showMessageDialog(null,
						    "ERROR: logic.Question fields cannot be empty",
						    "Error saving logic.Question",
						    JOptionPane.ERROR_MESSAGE);
                ...
                if (flag == false) {  // no match was found
				    JOptionPane.showMessageDialog(null,
					    "ERROR: logic.Question field does not appear as a Possible Answer.",
					    "Error saving logic.Question",
					    JOptionPane.ERROR_MESSAGE);
				    jtf.requestFocusInWindow();
				    return false;    // don't bother checking any further
	...
	private void codeFieldActionPerformed() {
	}

	private void optionsFieldMouseClicked() {
	}

	private void optionsFieldMousePressed(JTextField jtf) {
