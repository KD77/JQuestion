 ...
 * for the logic.JQuestions Editor application.
 *
...
public class JQuestionsEditorGUI extends JPanel {
    ...
    // Reference to presentation.JQuestionsEditor application.
    ...
    private String hintText;     // For current logic.Question
    private String explainText;    // For current logic.Question

    // GUI components.
    private final QuestionpoolAndsetValue configWindow;
    private JQEMultiChoiceQuestionScreen multiChoicePanel;
    private JQEDragAndDropQuestionScreen dndPanel;
    ...
    public JQuestionsEditorGUI(JQuestionsEditor jqTool) {
        ...
        // The config window.
        configWindow = new QuestionpoolAndsetValue();
    }
    ...
    private void updateCountLabel() {
        ...
        try {
            ...
            size = 0;
            countLabel.setText("logic.Question 0 of 0");
            return;
        } catch (Exception e) {
            e.printStackTrace();
            return;
        ...
        if (isCreatingNew) {
            if (size <= 0) {
                countLabel.setText("logic.Question 1 of 1");
                return;
            }
            if (size >= 1) {
                countLabel.setText("logic.Question " + (size + 1) + " of " + (size + 1));
                return;
            }
        } else {
            if (size <= 0) {
            ...
            if (size > 1) {
                countLabel.setText("logic.Question " + (currentRecordId + 1) + " of " + size);
                return;
            }
        }
    }
     ...
     * <p>
     * A logic.Question record is implicitly saved each time the user presses
     * the 'Next' or 'Previous'  buttons.
     *
     * @return - The saved logic.Question object.
     */
    private Question saveQuestionRecord() {
        ...
        } else if (newQuestionType == QuestionType.DRAG_N_DROP) {
    ...
    /**
     * Display the given logic.Question object. Choose the correct display panel
     * based on the data.QuestionType. Restore hint and explain text for the
     * logic.Question which may be edited by the user.
     *
     * @param question - The logic.Question to display.
     */
    ...
    /**
     * Check currently displayed logic.Question details and save any changes.
     */
    ...
    private void previousButtonActionPerformed() {
        ...
        // Need to check for any errors before we can save logic.Question object.
    ...
    public boolean doErrorChecking() {
        ...
        if (isCreatingNew) {
            ...
            } else if (newQuestionType == QuestionType.MULTI_CHOICE) {
            ...
            if (currentlyDisplayedQuestion.getQuestionType() == QuestionType.DRAG_N_DROP) {
                retVal = dndPanel.errorChecking();
            } else if (currentlyDisplayedQuestion.getQuestionType() == QuestionType.MULTI_CHOICE) {
                retVal = multiChoicePanel.errorChecking();
            }
        }
    ...
    /**
     * User has clicked 'new' button to create a new logic.Question record.
     */
    private void newButtonActionPerformed() {
        ...
        String questionTypeStr = (String) JOptionPane.showInputDialog(
                this,
                "What type of logic.Question would you like to create ?\n",
                "Create a new logic.Question Dialog",
        ...
        if ((questionTypeStr != null) && (questionTypeStr.length() > 0)) {
            ...
            } else if (questionTypeStr.compareTo((String) options[1]) == 0) {
                hintText = "";
                explainText = "";
    ...
    /**
     * User has clicked 'delete' button to delete the current logic.Question record.
     */
    private void deleteButtonActionPerformed() {
        ...
        if (isCreatingNew) {
            int value = JOptionPane.showOptionDialog(this,
                    "Delete the current logic.Question record ?",
                    "Delete logic.Question?", JOptionPane.YES_NO_CANCEL_OPTION,
        ...
        if (qPool.getSize() <= 0) {
            JOptionPane.showMessageDialog(this, "No logic.Question record to delete!");
            return;
        }
        ...
        if (value == JOptionPane.YES_OPTION) {
            ...
            if (retVal == true) {
                ...
                JOptionPane.showMessageDialog(this, "logic.Question has been deleted.");
            } else {
                JOptionPane.showMessageDialog(this, "Error deleting logic.Question has failed.");
            }
        }
    }
    ...
    private void hintButtonActionPerformed() {
        ...
        String response = MultiLineTextInputDialog.showInputDialog("Enter some hint text", hintText, true);
     ...
     * by the user and made available to the examinee at exam-time.
     */
    private void explainButtonActionPerformed() {
        MultiLineTextInputDialog multiDialog = new MultiLineTextInputDialog();
        String response = MultiLineTextInputDialog.showInputDialog("Enter some explanation text", explainText, true);
        if (response == null) {   // User pressed 'Cancel'
    ...
    private void menuAboutActionPerformed() {
        JQEAboutDialog jQEAboutDialog = JQEAboutDialog.getInstance();
        jQEAboutDialog.setVisible(true);
    }
    ...
    private void menuFileOpenActionPerformed(ActionEvent e, boolean append) {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (append) {
                qPool = jqEditor.getQuestionPool();
            } else if (!append) {  // Set new logic.QuestionPool
     ...
     * File Delete
     */
    private void menuDeleteMarkedActionPerformed() {
        QuestionPool qPool = jqEditor.getQuestionPool();
        ...
        if (numQs == 0) {
            ...
            questionString = "Delete the " + numQs + " marked logic.Question record ?";
        else if (numQs > 1)
            questionString = "Delete the " + numQs + " marked logic.Question records ?";
     ...
     */
    private void menuFileSaveMarkedActionPerformed() {

        QuestionPool qPool = jqEditor.getQuestionPool();

        if (qPool.getSize() <= 0) {
            JOptionPane.showMessageDialog(this, "No logic.Question record to save!");
            return;
        }
        ...
        if (numQs == 0) {
            ...
            questionString = "Save the " + numQs + " marked logic.Question record ?";
        else if (numQs > 1)
            questionString = "Save the " + numQs + " marked logic.Question records ?";
     ...
     */
    private void menuFileSaveActionPerformed() {
        int returnVal = fc.showSaveDialog(JQuestionsEditorGUI.this);
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            File file = fc.getSelectedFile();
            //Save the file.
     ...
     */
    public void menuInsertImageActionPerformed() {
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
            ...
            } else if (multiChoicePanel.isShowing()) {
     ...
     */
    public void menuConfigActionPerformed() {
        QuestionPool qPool = jqEditor.getQuestionPool();
        configWindow.setValues(qPool.getDescriptionText(),
                qPool.getIsChangeAllowed(),
                qPool.getIsExam(),
    ...
    private void createMenuBar() {
        ...
        menuFileSaveMarked.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                menuFileSaveMarkedActionPerformed();
            }
        });
        ...
        menuDeleteMarked.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                menuDeleteMarkedActionPerformed();
            }
        });
        ...
        menuInsertImage.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                menuInsertImageActionPerformed();
            }
        });
        ...
        // ---- menuAbout ----
        menuAbout.setText("About logic.JQuestions Editor");
    ...
    public JPanel createNorthPanel() {
        ...
        // ---- markQuestionBox ----
        markQuestionBox = new JCheckBox("Mark logic.Question");
    ...
    public JPanel createSouthPanel() {
        ...
        previousButton
                .setToolTipText("<html>Display previous logic.Question record (if one exists).</html>");
        ...
        nextButton.setToolTipText("<html>Display next logic.Question record (if one exists).</html>");
        ...
        newButton.setToolTipText("<html>Create a new logic.Question record.</html>");
        ...
        deleteButton.setToolTipText("<html>Delete the current logic.Question record.</html>");
        ...
        hintButton.setToolTipText("<html>Add hint-text to current logic.Question record.</html>");
        ...
        explainButton.setToolTipText("<html>Add answer and explanation text <br>to current logic.Question record.</html>");
        explainButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                explainButtonActionPerformed();
            }
        });
    ...
    private void initComponents() {
        ...
        multiChoicePanel = new JQEMultiChoiceQuestionScreen(northPanel, southPanel);
        dndPanel = new JQEDragAndDropQuestionScreen(northPanel, southPanel);
